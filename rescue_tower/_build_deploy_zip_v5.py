...
OUT = os.path.join(ROOT, "rescue_tower_v5_deploy.zip")
SKIP = {"__pycache__", ".next", "node_modules"}

DEPLOY_NOTE = """智能救援塔 V5 部署包（v4 + 禁航区）—— 先看这个

包里有两部分：
  1. v5/           —— 总控程序（必需）
  2. raspberry_pi/ —— 视觉程序 666.py + 模型 best.pt + 摄像头标定文件

V5 相比 V4 只多了一项：config.py 里配好了 10 块不能航行区域
（OBSTACLES_MM），A* 规划自动绕开。固件与 PID 参数和 V4 完全相同。

============================================================
一、如果朋友的树莓派已经在跑 V4
============================================================
只做一件事：
  把 v5/ 整个文件夹放到 ~/rescue_tower/ 下面（与 raspberry_pi/ 平级）：
    rescue_tower/
    ├── raspberry_pi/   （原有的，不动）
    ├── v4/             （可留着，也可删掉）
    └── v5/
  固件不用重烧、模型不用重传——V5 与 V4 固件相同。

============================================================
二、如果朋友的树莓派在跑 V3 或更早的旧系统
============================================================
  1. 把 v5/ 放到 ~/rescue_tower/ 下面（结构同上）。
  2. 把小船 ESP32 固件换成新的：用 Thonny 打开 v5/firmware/boat_main.py
     烧进小船（旧固件没有 /M 接口，自动航行必须换）。
     （升降台、水位固件如果已经在跑就不用动。）

============================================================
三、如果朋友的树莓派是全新的（什么都没有）
============================================================
  1. 把 v5/ 和 raspberry_pi/ 都放到 ~/rescue_tower/ 下面（结构如上）。
  2. 装视觉依赖（在 raspberry_pi/vision/ 下）：
       pip install ultralytics opencv-python numpy
     并在 666.py 提示时按需执行 yolo export 转换 NCNN（不转也能用 best.pt 跑）。
  3. 烧固件（小船必须烧 v5/firmware/boat_main.py）：
       boat_main.py -> 小船
       lift_main.py  -> 升降台
       water_main.py -> 水位传感器
...
============================================================
  ...
  OBSTACLES_MM           —— 禁航区已按现场布置配好，改场地时只改这里
"""
...
def main() -> None:
    note = os.path.join(ROOT, "_部署包说明_v5.txt")
    with open(note, "w", encoding="utf-8") as fh:
        fh.write(DEPLOY_NOTE)
    with zipfile.ZipFile(OUT, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.write(note, "部署说明-先看这个.txt")
        add_tree(zf, os.path.join(ROOT, "v5"), "v5")
