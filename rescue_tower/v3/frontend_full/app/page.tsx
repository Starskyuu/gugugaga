...
type VisionSnapshot = {
  ...
  mission?: { stage: string; stage_cn: string; message: string; target_id: string | null };
  water?: { value: number | null; baseline: number | null; rise: number | null; triggered: boolean; online: boolean };
  lift?: { command: string; ok: boolean | null; message: string };
};
...
export default function Home() {
  ...
  const missionAction = useCallback(async (path: string, label: string) => {
    try {
      ...
      addLog(`${label}失败，请检查树莓派服务`, 'danger');
    }
  }, [addLog, apiBase]);
      ...
      <section className="workspace">
        <section className="mission-panel">
          ...
          <div className="radar-view">
            ...
            <div className="direction-readout"><small>当前任务状态</small><strong>{activeCommand ? commandText[activeCommand] : (vision?.mission?.stage_cn ?? '安全停止')}</strong></div>
        ...
        <aside className="telemetry-panel">
          ...
          <div className="mission-actions">
            <button onClick={() => void missionAction('/api/mission/start', '自动任务已启动')}>开始自动任务</button>
            <button onClick={() => void missionAction('/api/mission/test', '测试任务已启动')}>跳过水位测试</button>
            <button onClick={() => void missionAction('/api/mission/reset', '系统已复位')}>复位</button>
          </div>
          ...
          <div className="metric-grid">
            ...
            <div className="metric"><span><Signal size={16} />水位 ADC</span><strong>{vision?.water?.value ?? '--'}</strong></div>
            <div className="metric"><span><ArrowUp size={16} />升降台</span><strong>{vision?.lift?.command ?? '--'}</strong></div>
          </div>
          {vision?.mission && <div className="mission-message"><strong>{vision.mission.stage_cn}</strong><p>{vision.mission.message}</p></div>}
