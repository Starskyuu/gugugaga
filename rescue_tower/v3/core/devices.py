...
def http_command(ip: str, command: str, require_echo: bool = False) -> tuple[bool, int | None, str | None]:
    """向旧版 ESP32 发一个单字母命令，返回成功、延迟和错误。"""
    started = time.monotonic()
    try:
        with urllib.request.urlopen(f"http://{ip}:{config.DEVICE_HTTP_PORT}/{command}", timeout=config.DEVICE_TIMEOUT_S) as response:
            body = response.read(32).decode("utf-8", "ignore").strip()
        latency = round((time.monotonic() - started) * 1000)
        # 小船返回单字母确认；升降台返回整个网页，只检查 HTTP 成功即可。
        if require_echo and body != command:
...
class Boat:
    ...
    def send(self, command: str) -> bool:
        ...
        ok, latency, error = http_command(config.BOAT_IP, command, require_echo=True)
